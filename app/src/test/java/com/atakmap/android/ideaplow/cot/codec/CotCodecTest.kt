...
class CotCodecTest {
    ...
    fun `pli detail round-trips`() {
        ...
        assertEquals(DetailNode.IDEAPLOW, node.name)
        ...
        val decoded = IdeaPlowDetail.fromNode(node)
    ...
    fun `invalid detail returns null`() {
        assertNull(IdeaPlowDetail.fromNode(DetailNode("something_else")))
        assertNull(IdeaPlowDetail.fromNode(DetailNode(DetailNode.IDEAPLOW))) // no vehicle child
    }
    ...
    fun `non-coverage detail decodes empty`() {
        assertTrue(CoverageCotCodec.decode(DetailNode(DetailNode.IDEAPLOW)).isEmpty())
    }
