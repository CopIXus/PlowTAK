...
import com.atakmap.android.plowtak.coverage.SegmentCodec
...
/**
 * Batches [TreatSegment]s into a compact `<__plowtak>` coverage detail for
...
object CoverageCotCodec {
    ...
    const val COVERAGE_EVENT_TYPE = "b-i-x-plowtak-cov"
    ...
    fun decode(node: DetailNode): List<TreatSegment> {
        ...
        else node.firstChild(DetailNode.PLOWTAK) ?: return emptyList()
        val coverage = plowtak.firstChild("coverage") ?: return emptyList()
