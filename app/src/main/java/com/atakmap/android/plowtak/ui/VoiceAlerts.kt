...
class VoiceAlerts(
    ...
    fun say(category: String, text: String, minGapMs: Long = DEFAULT_GAP_MS) {
        ...
        try {
            tts?.speak(text, TextToSpeech.QUEUE_ADD, null, "plowtak-$category-$now")
        } catch (e: Exception) {
    ...
    fun taskReceived(from: String) =
        say(CAT_TASK, "New task from $from. Check PlowTAK panel.", 10_000L)
    ...
    companion object {
        private const val TAG = "PlowTakVoice"
