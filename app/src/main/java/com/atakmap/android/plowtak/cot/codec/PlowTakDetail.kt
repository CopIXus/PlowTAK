...
/**
 * The `<__plowtak>` PLI detail: everything a receiving PlowTak client needs
...
data class PlowTakDetail(
    ...
    fun toNode(): DetailNode {
        ...
        return DetailNode(DetailNode.PLOWTAK, emptyMap(), children)
    }
    ...
    companion object {
        ...
        /** Returns null if the node is not a valid `<__plowtak>` detail. */
        fun fromNode(node: DetailNode): PlowTakDetail? {
            if (node.name != DetailNode.PLOWTAK) return null
