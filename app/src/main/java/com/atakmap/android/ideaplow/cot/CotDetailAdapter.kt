...
import com.atakmap.android.ideaplow.cot.codec.DetailNode
import com.atakmap.coremap.cot.event.CotDetail

/**
 * Bridges the framework-free [DetailNode] tree to ATAK's CotDetail. This is
 * the ONLY place IdeaPlow detail structures touch the SDK type — codecs stay
...
object CotDetailAdapter {
    ...
    /** Find the `<__ideaplow>` child of a CoT event's detail, if present. */
    fun findIdeaPlowNode(eventDetail: CotDetail?): DetailNode? {
        val detail = eventDetail ?: return null
        val child = detail.getFirstChildByName(0, DetailNode.IDEAPLOW) ?: return null
