...
/**
 * Owns one marker per remote fleet unit in the IdeaPlow MapGroup, styled by
 * vehicle type + status color, greyed out when stale. IdeaPlow PLI events
...
class FleetMarkerManager(
    ...
    private fun applyStaleness(marker: Marker, stale: Boolean) {
        if (stale) {
            ...
            marker.setMetaBoolean("ideaplow.stale", true)
        } else {
            marker.setMetaBoolean("ideaplow.stale", false)
        }
    }

    companion object {
        private const val TAG = "IdeaPlowFleet"
        private const val SUBGROUP = "Fleet"
        private const val MARKER_UID_PREFIX = "ideaplow-fleet-"
