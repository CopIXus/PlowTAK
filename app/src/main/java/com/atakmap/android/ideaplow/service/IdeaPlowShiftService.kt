package com.atakmap.android.ideaplow.service
 ...
 */
class IdeaPlowShiftService : Service() {
    ...
    private fun buildNotification(text: String): Notification {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            manager.createNotificationChannel(
                NotificationChannel(
                    CHANNEL_ID, "IdeaPlow shift", NotificationManager.IMPORTANCE_LOW
                ).apply {
        ...
        return Notification.Builder(this, CHANNEL_ID)
            .setContentTitle("IdeaPlow — shift active")
            .setContentText(text)
    ...
    companion object {
        private const val TAG = "IdeaPlowShiftService"
        private const val CHANNEL_ID = "ideaplow_shift"
        private const val NOTIFICATION_ID = 0x1DEA
        private const val EXTRA_TEXT = "text"

        /** Plugin package (applicationId), not the host's. */
        private const val PLUGIN_PACKAGE = "com.atakmap.android.ideaplow.plugin"

        fun start(context: Context, statusText: String) {
            try {
                val intent = Intent().apply {
                    component = ComponentName(PLUGIN_PACKAGE, IdeaPlowShiftService::class.java.name)
